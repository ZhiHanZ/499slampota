...
DEFINE_string(
    options, "register,user,chirp,reply,follow,read,monitor",
...
int main(int argc, char **argv) {
  ...
  if (FLAGS_user.size() > 0) {
    ...
    if (result == "illegal") {
      std::cout
          << "You are not a registered user, and that username is unavailable."
          << std::endl;
    }
    if (result == "success") {
      std::cout << username
                << " was not registered, but has now been registered."
                << std::endl;
      logged_in = true;
  ...
  if (FLAGS_follow.size() > 0) {
    if (logged_in) {
      ...
      if (result == "success") {
        std::cout << username << " is now following " << FLAGS_follow << "."
                  << std::endl;
      } else {
        std::cout << "The user you are trying to follow does not exist."
                  << std::endl;
      }
    } else {
      std::cout << "Please login with a username." << std::endl;
  ...
  if (FLAGS_read.size() > 0) {
    if (logged_in) {
      std::vector<chirp::Chirp> vector_to_read =
          service_layer_client.Read(FLAGS_read);
      if (vector_to_read.empty()) {
        ...
        for (unsigned int i = 0; i < vector_to_read.size(); i++) {
          ...
          std::cout << "with a chirp id of: [" << vector_to_read[i].id() << "]"
                    << std::endl;
          std::cout << std::endl;
        }
      }
