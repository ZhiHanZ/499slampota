...
class KeyValueServer final : public chirp::KeyValueStore::Service {
 public:
  // function call to the lowest-level key-value backend's version of Put, which
  // inserts into the map
  grpc::Status put(grpc::ServerContext* context,
                   const chirp::PutRequest* request,
                   chirp::PutReply* reply) override;
  // function call to the lowest-level key-value backend's version of Get, which
  // retreives from the map
  grpc::Status get(grpc::ServerContext* context,
                   grpc::ServerReaderWriter<chirp::GetReply, chirp::GetRequest>*
                       stream) override;
  // function call to the lowest-level key-value backend's version of DeleteKey,
  // which inserts into the map
